...
| `measurement-conversion` | 22/2 | **Projection:** prompts, answer/explanation prose, equation strings, relative-size statements, column headers, and hidden table rows. **Canonical:** units, factor, quantities, equivalence, and table rows. The view chooses which values are withheld. |
...
| `place-value-arithmetic` | 21/2 | **Projection:** step explanations and regrouping statements. **Witness:** operands, digit decomposition, regrouping evidence, and typed strategy steps remain; equation and prose fields become view-derived. |
...
| `measurement-conversion` | `measure-conversion`, `measure-conversion-table` |
...
| `time` | 17 | `time-analog`, `time-digital` | **Derive:** retain a typed instant/seconds value and period; derive `HH:MM:SS` formatting in the view/shared time presenter. |
