...
const toClockTime = (minutesSinceMidnight: number): {hour: number; minute: number} => {
    if (!Number.isInteger(minutesSinceMidnight)) {
        throw new ViewValidationError('time-elapsed', 'Time instants must be whole minutes since midnight.');
    }
    const hour = Math.floor(minutesSinceMidnight / 60);
    const minute = minutesSinceMidnight % 60;
    if (hour < 1 || hour > 11 || minute < 0 || minute > 59) {
        throw new ViewValidationError('time-elapsed', `Unsupported minute instant: ${minutesSinceMidnight}`);
    }
    return {hour, minute};
};
...
function AnalogClock({time, label}: {time: number; label: string}) {
    const clockTime = toClockTime(time);
    const formattedTime = displayTime(clockTime);
    const angles = getClockAngles(`${formattedTime}:00`);
    const ticks = getTickMarks();
    return (
        <div className="flex flex-col items-center gap-2">
            <div className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{label}</div>
            <svg className="h-[172px] w-[172px]" viewBox="0 0 100 100" role="img" aria-label={`${label} analog clock showing ${formattedTime}`}>
            ...
            <div className="font-mono text-xl font-bold text-slate-800">{formattedTime}</div>
        </div>
    );
}
...
const TimeElapsedCore = ({config, payload}: CoreProps) => {
    ...
    const start = toClockTime(data.startMinutesSinceMidnight);
    const end = toClockTime(data.endMinutesSinceMidnight);
    const coherent = data.crossesHour === true
        ...
        && data.elapsedMinutes === data.endMinutesSinceMidnight - data.startMinutesSinceMidnight
    ...
    return (
        <div className="w-[760px] rounded-2xl bg-white p-7 font-sans shadow-[0_10px_32px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-5 flex items-center justify-center gap-12">
                <Clock time={data.startMinutesSinceMidnight} label="Start" />
                <div className="text-4xl font-light text-slate-300">→</div>
                <Clock time={data.endMinutesSinceMidnight} label="End" />
            </div>
