...
import {formatDayPeriod} from './time-presentation.ts';
import {
    formatDigitalTime,
    formatTimeClue,
    validateDigitalTimeProblem
} from './time-digital/helpers.ts';

export type TimeDigitalViewId = 'time-digital' | 'time-digital-construction';

interface TimeDigitalViewProps {
    mode: 'reading' | 'construction';
    payload: ViewRenderPayload<TimeDigitalViewId>;
...
export const TimeDigitalView = ({mode, payload, viewId}: TimeDigitalViewProps) => {
    const {problem, isSolutionView} = payload;
    const data = problem.data;
    const parts = validateDigitalTimeProblem(data, viewId);
    const formattedTime = formatDigitalTime(parts);
    const periodLabel = formatDayPeriod(data.period);
    const answer = periodLabel === undefined ? formattedTime : `${formattedTime} ${periodLabel}`;
    const isReading = mode === 'reading';
    const displayValue = isReading || isSolutionView
        ? formattedTime
    ...
    return (
        <div className="flex w-full items-center justify-center bg-white p-6 font-sans">
            <div className="flex w-[520px] flex-col items-center gap-6 rounded-3xl border border-slate-200 bg-slate-50 px-8 py-7 shadow-sm">
                ...
                <DigitalDisplay value={displayValue} period={periodLabel} highlighted={isSolutionView} />
                {isReading && (
