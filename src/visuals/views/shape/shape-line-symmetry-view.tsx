...
interface ShapeLineSymmetryViewProps {
    mode: ShapeLineSymmetryTaskMode;
    payload: RenderPayload<AbstractProblem<ShapeLineSymmetryProblem>>;
    viewId: string;
}

export const ShapeLineSymmetryView = ({
    mode,
    payload,
    viewId
}: ShapeLineSymmetryViewProps) => {
    const {problem, isSolutionView, seed} = payload;
    const data = problem.data;
    validateProblemData(viewId, data, ['identification', 'drawing']);
    validateProblemData(viewId, data.identification, [
        'options',
        'answerIds'
    ]);
    validateProblemData(viewId, data.drawing, [
        'figure',
        'completedAxes'
    ]);
