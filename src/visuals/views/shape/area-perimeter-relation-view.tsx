...
export type AreaPerimeterRelationViewId =
    | 'area-perimeter-comparison'
    | 'area-perimeter-construction';

interface AreaPerimeterRelationViewProps {
    mode: 'comparison' | 'construction';
    payload: ViewRenderPayload<AreaPerimeterRelationViewId>;
    viewId: AreaPerimeterRelationViewId;
}
...
function validateRelation(
    viewId: AreaPerimeterRelationViewId,
    data: AreaPerimeterRelationProblem
) {
    validateProblemData(viewId, data, ['relation', 'first', 'second']);
    const validMeasures = data.relation === 'equal-perimeter'
        ? data.first.perimeter === data.second.perimeter
            && data.first.area !== data.second.area
        : data.first.area === data.second.area
            && data.first.perimeter !== data.second.perimeter;

    if (!validRectangle(data.first) || !validRectangle(data.second) || !validMeasures) {
...
export const AreaPerimeterRelationView = ({
    mode,
    payload,
    viewId
}: AreaPerimeterRelationViewProps) => {
    const {problem, isSolutionView} = payload;
    const data = problem.data;
    validateRelation(viewId, data);
    const samePerimeter = data.relation === 'equal-perimeter';
    const isConstruction = mode === 'construction';
    const equalValue = samePerimeter ? data.first.perimeter : data.first.area;
    const equalUnit = samePerimeter ? 'units' : 'square units';
    ...
    return (
        <div className="w-[740px] rounded-2xl bg-white p-7 font-sans shadow-[0_10px_30px_rgba(0,0,0,0.05)]">
            <div className="text-center text-[1.15rem] font-bold leading-snug text-slate-700">
                {isConstruction
            ...
            <div className={`mt-4 min-h-[60px] rounded-xl border-2 px-5 py-3 text-center font-mono text-[1.05rem] font-bold ${
                ...
                {isConstruction && !isSolutionView
                    ...
                    : isSolutionView
                        ? samePerimeter
                            ? `Perimeters: ${data.first.perimeter} = ${data.second.perimeter} units. Areas: ${data.first.area} ≠ ${data.second.area} square units.`
                            : `Areas: ${data.first.area} = ${data.second.area} square units. Perimeters: ${data.first.perimeter} ≠ ${data.second.perimeter} units.`
                        : samePerimeter
                            ? `Both perimeters are ${data.first.perimeter} units. Are the areas equal?`
                            : `Both areas are ${data.first.area} square units. Are the perimeters equal?`}
            </div>
        </div>
    );
};
