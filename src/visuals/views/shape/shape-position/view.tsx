...
import {ShapePositionViewConfig, ShapePositionViewSchema} from './spec.ts';
import {withConfig} from '../../withConfig.tsx';
import {validateProblemData, ViewValidationError} from '../../../helpers/validation.ts';
import '../../../tailwind.css';

interface CoreProps {
    config: ShapePositionViewConfig;
    payload: ViewRenderPayload<'shape-position'>;
}
...
export const ShapePosition = withConfig(ShapePositionViewSchema, ShapePositionCore);

let root: ReturnType<typeof createRoot> | null = null;

window.renderView = (payload: ViewRenderPayload<'shape-position'>) => {
    const container = document.getElementById('view');
    if (container) {
        if (!root) {
            root = createRoot(container);
        }
        root.render(<ShapePosition payload={payload} />);
    }
};
