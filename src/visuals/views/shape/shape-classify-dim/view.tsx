...
import {ShapeClassifyDimViewConfig, ShapeClassifyDimViewSchema} from './spec.ts';
...
function ShapeSVG({ shape, size = 100 }: { shape: string; size?: number }) {
    ...
    if (shape === 'circle') {
        ...
        return (
            <svg {...commonProps}>
                <circle cx="50" cy="50" r="35" fill="url(#sphere-grad-viewer-react-classify)" stroke="#1d4ed8" strokeWidth="2" />
                <defs>
                    <radialGradient id="sphere-grad-viewer-react-classify" cx="30%" cy="30%" r="70%">
    ...
    throw new ViewValidationError('shape-classify-dim', `Unsupported shape: ${shape}`);
}

interface CoreProps {
    config: ShapeClassifyDimViewConfig;
...
const ShapeClassifyDimCore = ({ config: _config, payload }: CoreProps) => {
    ...
    const getLabelText = (opt: string) => {
        return opt === '2d' ? 'Flat (2D)' : 'Solid (3D)';
    };
...
export const ShapeClassifyDim = withConfig(ShapeClassifyDimViewSchema, ShapeClassifyDimCore);
