...
function OptionRow({options, isSolutionView}: {options: readonly ShapeCompositionOption[]; isSolutionView: boolean}) {
    return (
        <div className="flex flex-wrap gap-3 w-full justify-center">
            {options.map(option => (
                <div key={option.id} className={optionClass(option.correct, isSolutionView)}>
                    {option.label}
                </div>
            ))}
        </div>
...
function SingleLevelLayout({
    data,
    options,
    isSolutionView
}: {
    data: ShapeComposeShapesProblem;
    options: readonly ShapeCompositionOption[];
    isSolutionView: boolean;
}) {
    const target = data.compositionTree.shape;
    const promptText = `Which pieces can you join to make a ${shapeCompositionLabel(target)}?`;
            ...
            <div className="flex flex-col items-center w-[480px]">
                ...
                <div className="flex justify-center items-center w-[420px] h-[220px] bg-slate-50 border-2 border-slate-200 rounded-xl mb-[25px] p-[15px] box-border">
                    <div className="flex flex-col items-center gap-3">
                        ...
                        />
                        <span className="font-bold text-slate-500 uppercase">Target: {shapeCompositionLabel(target)}</span>
                    </div>
                </div>

                <OptionRow options={options} isSolutionView={isSolutionView} />
            </div>
        </div>
    );
}

function PrimitiveGlyph({shape}: {shape: ShapeCompositionShapeId}) {
    const glyph = shapeCompositionGlyph(shape);
...
function PrimitiveToken({shape, count}: {shape: ShapeCompositionShapeId; count: number}) {
    return (
        <div className="relative flex flex-col items-center justify-center w-[82px] h-[58px] rounded-lg border border-slate-200 bg-white px-1">
            ...
            <span className="text-[0.68rem] leading-tight text-center font-semibold text-slate-600">{shapeCompositionLabel(shape)}</span>
        </div>
    );
}

const groupInputs = (inputs: ShapeCompositionNode[]): Array<{shape: ShapeCompositionShapeId; count: number}> => {
    const counts = new Map<ShapeCompositionShapeId, number>();
    for (const input of inputs) counts.set(input.shape, (counts.get(input.shape) ?? 0) + 1);
    return [...counts].map(([shape, count]) => ({shape, count}));
};

function IntermediateDiagram({
    shape,
    componentShape,
    showSeams
}: {
    shape: ShapeCompositionShapeId;
    componentShape: ShapeCompositionShapeId;
    showSeams: boolean;
}) {
    return (
        <CompositionDiagram
            target={shapeCompositionDiagramTarget(shape)}
    ...
    return (
        <div className="relative flex items-center gap-2 bg-slate-50 border-2 border-slate-200 rounded-xl p-2.5 pt-8 min-w-[278px]">
            ...
            <div className="flex flex-col items-center">
                ...
                <span className="text-[0.68rem] text-slate-600">{shapeCompositionLabel(node.shape)}</span>
            </div>
        </div>
    );
}
...
function MultiLevelLayout({
    ...
    const target = data.compositionTree.shape;
    const promptText = `Which pieces can you build first, then join to make a ${shapeCompositionLabel(target)}?`;
                ...
                <div className="flex items-stretch gap-3 w-full min-h-[310px] mb-5">
                    ...
                    <section className={`w-[245px] border-2 rounded-xl p-3 flex flex-col items-center ${isSolutionView ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                        ...
                        <CompositionDiagram
                            ...
                            height={154}
                        />
                        <span className="font-bold text-slate-600 uppercase text-[0.78rem]">Final: {shapeCompositionLabel(target)}</span>
                    </section>
                </div>
...
export const ShapeComposeShapesCore = ({config: _config, payload}: CoreProps) => {
    ...
    const options = shapeCompositionOptions(data, seed);

    return data.compositionDepth === 1
        ? <SingleLevelLayout data={data} options={options} isSolutionView={isSolutionView} />
        : <MultiLevelLayout data={data} options={options} isSolutionView={isSolutionView} />;
};

interface DiagramProps {
    target: ShapeCompositionTargetId;
    componentShape: ShapeCompositionShapeId;
    showSeams: boolean;
    width?: number;
    height?: number;
...
const CompositionDiagram = ({target, componentShape, showSeams, width = 230, height = 160}: DiagramProps) => {
    ...
    const componentTarget = shapeCompositionDiagramTarget(componentShape);
    const usesRectangularInputs = componentTarget === 'rectangle' || componentTarget === 'square';

    return (
        <svg width={width} height={height} viewBox="0 0 220 160" role="img" aria-label={`${shapeCompositionLabel(target)} composition diagram`}>
            {target === 'rectangle' && <>
