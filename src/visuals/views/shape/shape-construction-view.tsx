...
import {ShapeConstructionProblem, ShapeCountAttribute, ShapeDefinition} from '../../../types/problems.ts';
...
export const ShapeConstructionView = ({payload}: {
    payload: RenderPayload<AbstractProblem<ShapeConstructionProblem>>;
}) => {
    const { problem, isSolutionView } = payload;
    validateProblemData('shape-build-shape', problem.data, ['kind', 'target']);
    const data = problem.data;
    const {target} = data;
    const sides = data.kind === 'attribute-count' ? data.sides : data.definition.sideCount;
    const corners = data.kind === 'attribute-count' ? data.corners : data.definition.vertexCount;
    ...
    if (data.kind === 'attribute-count') {
        validateProblemData('shape-build-shape', data, ['attribute', 'requiredCount']);
    ...
    if (data.kind === 'polygon-definition') {
        validateProblemData('shape-build-shape', data, ['definition']);
    ...
    throw new ViewValidationError('shape-build-shape', `Unsupported construction task: ${(data as {kind: string}).kind}`);
};
