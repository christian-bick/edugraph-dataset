...
function validateProperty(data: ArithmeticPatternProblem) {
    ...
    if (!data.propertyLaw
        ...
        fail('Invalid property explanation payload.');
    }
}

function validateGrade4(data: ArithmeticPatternExplainFeatureProblem) {
    validateProblemData(VIEW_ID, data, [
        'startValue',
        'ruleOperation',
        'ruleValue',
    ...
    if (!['add', 'multiply', 'multiply-position'].includes(data.ruleOperation)
        ...
        fail('The Grade 4 feature explanation does not agree with its supplied terms or rule.');
    }
}

function validateGrade4Practice(data: ArithmeticPatternGenerateProblem) {
    validateProblemData(VIEW_ID, data, [
    ...
    validateProblemData(VIEW_ID, data, ['missingTermIndex']);
    if (!Number.isInteger(data.missingTermIndex)
        ...
        fail('The generated-term response is inconsistent.');
    }
}
...
const OperationsPatternExplanationCore = ({config, payload}: CoreProps) => {
    ...
    if (data.task === 'identify-feature') return fail('Feature-identification payloads belong to the pattern-table view.');
