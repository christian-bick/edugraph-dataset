...
const OperationsMultiplicativeComparisonCore = ({config: _config, payload}: CoreProps) => {
    ...
    if (data.operation !== 'multiplication') {
        throw new ViewValidationError(
            VIEW_ID,
            'This interpretation layout requires a multiplication comparison.'
        );
    }

    const presentation = presentMultiplicativeComparison(data, 'compared');
    const referenceWidth = MODEL_WIDTH / data.scaleFactor;
    const questionStatement = presentation.comparisonStatement.replace(
        ...
        <div className="w-[790px] rounded-2xl bg-white p-8 font-sans shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-6 grid grid-cols-3 gap-4">
                <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-center">
                    ...
                    <div className="text-sm font-semibold text-sky-800">{presentation.referenceEntity}</div>
                </div>
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-center">
                    ...
                    <div className="text-sm font-semibold text-amber-800">{presentation.comparedEntity}</div>
                </div>
            </div>

            <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-5">
                <div className="grid grid-cols-[120px_1fr] items-center gap-x-4 gap-y-4">
                    <div className="text-right text-sm font-bold text-sky-800">{presentation.referenceEntity}</div>
                    ...
                    <div className="text-right text-sm font-bold text-amber-800">{presentation.comparedEntity}</div>
                    <div>
                        <div
            ...
            <div className="mt-5 rounded-xl border-2 border-indigo-200 bg-indigo-50 px-5 py-4 text-center">
                ...
                <div className="mt-1 font-mono text-[2rem] font-extrabold text-indigo-950">{presentation.solutionEquation}</div>
            </div>
            ...
            <div className="mt-5 min-h-[76px] rounded-xl border-2 border-emerald-300 bg-emerald-50 px-5 py-4 text-center">
                ...
                {isSolutionView ? (
                    <div className="mt-1 text-xl font-bold text-emerald-950">{presentation.comparisonStatement}</div>
                ) : (
