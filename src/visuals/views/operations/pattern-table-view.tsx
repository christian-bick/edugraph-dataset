...
function validateGrade4Base(viewId: string, data: Exclude<ArithmeticPatternProblem, ArithmeticPatternLegacyProblem>) {
...
function validateGenerate(viewId: string, data: ArithmeticPatternGenerateProblem) {
    validateProblemData(viewId, data, ['missingTermIndex', 'response']);
    if (!Number.isInteger(data.missingTermIndex)
        ...
        fail(viewId, 'The generated-term response must be the supplied term at the requested position.');
    }
}

function validateIdentify(viewId: string, data: ArithmeticPatternIdentifyFeatureProblem) {
    validateProblemData(viewId, data, [
    ...
    if (data.featureOptions.length !== 3
        ...
        fail(viewId, 'The supplied feature response must identify exactly one listed feature.');
    }
}
...
export const PatternTableView = ({mode, payload, viewId}: PatternTableViewProps) => {
    ...
    if (data.task === undefined) {
        if (mode !== 'legacy-classification') {
            fail(viewId, 'Retained table payloads require the legacy-classification view.');
        }
        return <LegacyTable data={data} isSolutionView={isSolutionView} />;
    }

    if (data.task === 'generate') {
        if (mode !== 'generation') fail(viewId, 'Generated-term payloads require the generation view.');
        validateGrade4Base(viewId, data);
        validateGenerate(viewId, data);
    } else if (data.task === 'identify-feature') {
        if (mode !== 'feature-classification') {
            fail(viewId, 'Feature-identification payloads require the feature-classification view.');
        }
        validateGrade4Base(viewId, data);
        validateIdentify(viewId, data);
    } else {
        return fail(viewId, 'This view supports generated terms and feature identification only.');
    }
