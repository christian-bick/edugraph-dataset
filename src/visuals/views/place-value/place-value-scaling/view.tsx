...
const PlaceValueScalingCore = ({config: _config, payload}: CoreProps) => {
    ...
    const presentation = placeValueScalingPresentation(data);
    ...
    return (
        <div className="w-[780px] rounded-2xl bg-white p-7 font-sans shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-2 text-center text-xl font-bold leading-relaxed text-slate-800">{presentation.prompt}</div>
            ...
            <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 text-center">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-indigo-200 bg-white px-3 py-3 font-mono text-xl font-extrabold text-indigo-950">
                        {isSolutionView ? presentation.multiplicationEquation : presentation.questionMultiplicationEquation}
                    </div>
                    <div className="rounded-lg border border-amber-200 bg-white px-3 py-3 font-mono text-xl font-extrabold text-amber-950">
                        {isSolutionView ? presentation.divisionEquation : presentation.questionDivisionEquation}
                    </div>
                </div>
            </div>

            {isSolutionView ? (
                <div className="mt-4 rounded-xl border-2 border-emerald-500 bg-emerald-50 px-5 py-4 text-center text-lg font-bold text-emerald-950">{presentation.comparisonStatement}</div>
            ) : (
