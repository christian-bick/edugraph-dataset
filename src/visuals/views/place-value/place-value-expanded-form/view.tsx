...
function PlaceStrip({data}: {data: MultiDigitPlaceValueExpandedProblem}) {
    const placeWidth = data.placeValues.length === 7 ? 'w-[88px]' : 'w-[108px]';
    return (
        <div className="mt-5 flex justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-4">
            {data.placeValues.map(place => (
                <div className={`${placeWidth} text-center`} key={place.name}>
...
function Grade4ExpandedForm({data, isSolutionView}: {
    ...
    return (
        <div className="w-[780px] rounded-2xl bg-white p-7 font-sans shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-2 text-center text-xl font-bold text-slate-800">Write the numeral as a sum of its nonzero place values.</div>
            ...
            {isSolutionView ? (
                <div className="mt-5 rounded-xl border-2 border-emerald-500 bg-emerald-50 px-5 py-5 text-center font-mono text-xl font-extrabold text-emerald-950">{formatExpandedEquation(data.number, data.terms)}</div>
            ) : (
...
const PlaceValueExpandedFormCore = ({config: _config, payload}: CoreProps) => {
    ...
    if (isGrade4(data)) {
        validateProblemData(VIEW_ID, data, ['task', 'placeValues']);
