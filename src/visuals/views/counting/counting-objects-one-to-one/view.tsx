...
import { CountingObjectsOneToOneViewConfig, CountingObjectsOneToOneViewSchema } from './spec.ts';
...
interface CoreProps {
    config: CountingObjectsOneToOneViewConfig;
    payload: ViewRenderPayload<'counting-objects-one-to-one'>;
}

const CountingObjectsOneToOneCore = ({ config, payload }: CoreProps) => {
    const { problem, isSolutionView } = payload;
    const seed = payload.seed;
    const data = problem.data;

    validateProblemData('counting-objects-one-to-one', data, ['numObjects']);
                ...
                <div className="relative w-[450px] h-[300px] bg-slate-50 border-2 border-slate-200 rounded-xl overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
                    {positions.map((pos, i) => (
                        ...
                        >
                            ...
                            <div className={`absolute -bottom-[16px] bg-white border-[1.5px] rounded-full w-5 h-5 text-[0.8rem] font-bold flex justify-center items-center shadow-[0_2px_4px_rgba(0,0,0,0.05)] ${
...
export const CountingObjectsOneToOne = withConfig(CountingObjectsOneToOneViewSchema, CountingObjectsOneToOneCore);
