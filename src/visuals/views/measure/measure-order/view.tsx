...
const MeasureOrderCore = ({config: _config, payload}: CoreProps) => {
    ...
    validateProblemData('measure-order', data, ['magnitudes', 'direction']);
    if (data.magnitudes.length !== 3
        || data.magnitudes.some(value => !Number.isFinite(value) || value <= 0)
        || data.magnitudes[0] >= data.magnitudes[1]
        || data.magnitudes[1] >= data.magnitudes[2]
        || !['ascending', 'descending'].includes(data.direction)) {
        throw new ViewValidationError(
            'measure-order',
            'Expected three canonically ordered distinct magnitudes and a supported direction.'
        );
    }
    const presentation = resolveMeasurementOrderPresentation(data, payload.seed);
        ...
        <div className="flex justify-center items-center p-8 bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.05)] w-fit">
            <div className="flex flex-col items-center w-[520px]">
                ...
                <div className="flex items-center gap-3 mt-6">
                    {presentation.answerIds.map((id, index) => (
                        <div key={id} className="flex items-center gap-3">
                            <div className={`w-14 h-14 rounded-xl border-2 flex items-center justify-center text-xl ${answerClass}`}>
                                {isSolutionView ? id : ''}
                            </div>
                            {index < presentation.answerIds.length - 1 && <span className="text-2xl text-slate-400">→</span>}
                        </div>
                    ))}
                </div>
            </div>
