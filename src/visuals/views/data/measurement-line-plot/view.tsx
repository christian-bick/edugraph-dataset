...
import {MeasurementLinePlotViewSchema} from './spec.ts';
import '../../../../tailwind.css';
...
const MeasurementLinePlotCore = ({payload}: CoreProps) => {
    ...
    return (
        <div className="w-[720px] rounded-2xl bg-white p-8 font-sans shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-1 text-xl font-bold text-slate-800">
                {isSolutionView ? 'Completed line plot' : 'Plot each measurement on the line plot.'}
            </div>
            <div className="mt-5 flex flex-wrap gap-2">
                {data.observations.map(({object, length}) => (
                    <div key={object} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold capitalize text-slate-700">
                        {object}: <span className="font-mono font-extrabold">{length} cm</span>
                    </div>
                ))}
            </div>
            <div className="mt-7 rounded-xl border border-slate-200 bg-slate-50 px-7 pb-5 pt-7">
                ...
                <div className="grid grid-cols-9">
                    {Array.from({length: 9}, (_, index) => index + 2).map(value => (
                        <div key={value} className="pt-2 text-center font-mono text-sm font-bold text-slate-700">{value}</div>
                    ))}
                </div>
                <div className="mt-2 text-center text-xs font-bold uppercase tracking-wider text-slate-500">Length (cm) · scale steps by 1</div>
            </div>
        </div>
    );
};
