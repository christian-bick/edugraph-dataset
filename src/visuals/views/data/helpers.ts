...
export function validateMeasurementData(data: MeasurementDataProblem, viewId: string) {
    ...
    if (data.unit !== 'cm' && data.unit !== 'in') {
        throw new ViewValidationError(viewId, 'Expected centimeters or inches.');
    }
    ...
    if (data.subdivisions === 1) {
        if (data.observations.some(({value}) => !Number.isInteger(value) || value < 2 || value > 10)) {
            throw new ViewValidationError(viewId, 'Whole-unit lengths must be integers from 2 through 10.');
        }
        return;
    }
    if (data.subdivisions === 4) {
        ...
        if (quarterUnits.some(value => !Number.isInteger(value) || value < 8 || value > 32)
            ...
            throw new ViewValidationError(viewId, 'Quarter-unit lengths must include half- and quarter-unit data from 2 through 8.');
        }
        return;
    }
    if (data.subdivisions === 8) {
        ...
        if (eighthUnits.some(value => !Number.isInteger(value) || value < 8 || value > 32)
            ...
            throw new ViewValidationError(viewId, 'Eighth-unit lengths must include eighth-, quarter-, and half-unit data from 1 through 4.');
        }
        return;
    }
...
export function formatMeasurementValue(length: number, unit: MeasurementDataProblem['unit']): string {
    if (unit === 'cm' && Number.isInteger(length)) return String(length);
