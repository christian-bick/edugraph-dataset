...
// TODO [1.OA.A.1]: Unknowns in all positions are not expressible; the arithmetic
// generator and word-problem view only produce result-unknown problems.
...
// TODO [1.OA.A.2]: Not coverable. The arithmetic generator only produces binary
// operations; there is no support for three addends. Reference permutation:
...
// TODO [1.OA.B.3]: Not coverable. No generator/view exercises arithmetic laws
// (Area.CommutativeLaw / Area.AssociativeLaw). Reference permutation:
...
// TODO [1.OA.B.4]: Only the make-ten complement case (Area.Difference within 10) is
// covered; general unknown-addend problems within 20 (e.g. 5 + ? = 13) have no
// generator support beyond the ten-complement.
...
// --- 1.OA.C.5: Relate counting to addition and subtraction (counting on/back) ---
...
// TODO [1.OA.D.7]: Not coverable. No generator/view produces true/false equation
// judgment tasks (Ability.PlausibilityEvaluation). Reference permutation:
...
// TODO [1.OA.D.8]: The operations-boxes view only leaves the result blank; unknowns
// in the first or second operand position (8 + ? = 11) are not supported.
...
// --- 1.NBT.A.1: Count to 120 (extending the count sequence as numeric order) ---
// TODO [1.NBT.A.1]: The range 100-120 is not expressible (ordering/counting generators
// cap at Scope.NumbersSmaller100), so permutations stop below 100.
...
// TODO [1.NBT.A.1]: The writing generator declares a range cap of Scope.NumbersSmaller10;
// numerals up to 120 are far out of reach.
...
// TODO [1.NBT.A.1]: Object counts beyond 20 are not supported by the counting
// generator (caps at Scope.NumbersSmaller20).
...
// TODO [1.NBT.B.2a]: Not coverable. No generator/view exercises bundling ten ones into
// a "ten" (Area.PlaceValue with exactly 10). Reference permutation:
...
// --- 1.NBT.B.2b: Teen numbers composed of a ten and further ones ---
...
// --- 1.NBT.B.2c: Multiples of 10 as a number of tens (and 0 ones) ---
// TODO [1.NBT.B.2c]: Not coverable. There is no way to express "multiples of 10" via
// ontology labels and no generator producing tens-bundle decompositions of 10-90.
...
// --- 1.NBT.B.3: Compare two two-digit numbers with >, =, < ---
...
// --- 1.NBT.C.4: Add within 100 (two-digit plus one-digit / multiple of 10) ---
// TODO [1.NBT.C.4]: The operand-shape restrictions (two-digit + one-digit, two-digit +
// multiple of 10) are not expressible via ontology labels; permutations cover general
// addition within 100.
...
// TODO [1.NBT.C.5]: Not coverable. The counting-inc-dec generator only steps by one and
// caps at Scope.NumbersSmaller20; ten-jumps within 100 are not supported. Reference
// permutation:
...
//         Scope.NumbersWithoutNegatives,
//         Scope.NumbersSmaller100,
//         Scope.DerivedCount, // stepping by tens
//         Ability.ProcedureExecution
//     ])
//     .applyLabelVariants([
//         [Area.Increment],
...
// TODO [1.NBT.C.6]: Not coverable. "Multiples of 10" operands are not expressible via
// ontology labels and the arithmetic generator has no such constraint. Reference
// permutation:
...
// --- 1.MD.A.1: Order three objects by length; compare indirectly via a third ---
// TODO [1.MD.A.1]: Not coverable. The measurement-compare generator only compares two
// objects; ordering three objects and indirect (transitive) comparison are not
// supported. Reference permutation:
...
// --- 1.MD.A.2: Express length as a whole number of iterated length units ---
...
// TODO [1.MD.B.3]: Digital clocks (Scope.DigitalClock) have no view; and there is no
// scope label restricting minutes to half-hour granularity (Scope.MinuteIntervals
// generates arbitrary minute times).
...
// TODO [1.G.A.1]: Distinguishing defining vs. non-defining attributes (closed/
// three-sided vs. color/orientation/size) has no generator/view support; only the
// build/draw competencies are covered.
...
// TODO [1.G.A.3]: Not coverable. No generator/view supports partitioning shapes into
// equal shares (halves/fourths/quarters). Reference permutation:
