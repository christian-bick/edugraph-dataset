...
function uniqueCounts(): number[] {
    ...
    const values: number[] = [];
    while (values.length < categoryIds.length) {
...
export class StatisticalGraphsGenerator implements ProblemGenerator<StatisticalGraphProblem, StatisticalGraphsGeneratorConfig> {
    ...
    generate(config: StatisticalGraphsGeneratorConfig): ProblemStub<StatisticalGraphProblem> {
        ...
        if (findTotal) {
            return {
                data: {
                    ...base,
                    operation: 'addition',
                    operandCategoryIds: categories.map(category => category.id) as [
                        StatisticalCategoryId,
                        StatisticalCategoryId,
                        StatisticalCategoryId
                    ],
        ...
        let operandCategories: [StatisticalCategory, StatisticalCategory] = [categories[0], categories[1]];
        if (config.useSubtraction && categories[0].count < categories[1].count) {
            operandCategories = [categories[1], categories[0]];
        }
        const [firstCategory, secondCategory] = operandCategories;
        ...
        return {
            data: {
                ...base,
                operation,
                operandCategoryIds: [firstCategory.id, secondCategory.id],
                answer: operation === 'addition'
                    ? firstCategory.count + secondCategory.count
                    : firstCategory.count - secondCategory.count
            }
        };
    }
