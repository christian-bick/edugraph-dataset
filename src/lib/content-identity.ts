...
function withoutNamedDependency(value: unknown, dependency: string): unknown {
    if (Array.isArray(value)) return value.map(item => withoutNamedDependency(item, dependency));
    if (!value || typeof value !== 'object') return value;
    const record = value as Record<string, unknown>;
    return Object.fromEntries(radixSortUtf8(Object.keys(record))
        .filter(key => key !== dependency && key !== `node_modules/${dependency}`)
        .map(key => [key, withoutNamedDependency(record[key], dependency)]));
}

/** Hashes runtime/toolchain package state while excluding one separately tracked semantic dependency. */
export function hashPackageStateWithoutDependency(
    projectRoot: string,
    dependency: string
): string {
    ...
    const packageLock = (existsSync(lockPath)
        ? JSON.parse(readFileSync(lockPath, 'utf-8'))
        : {}) as {
        lockfileVersion?: unknown;
        packages?: unknown;
    };
    return digestIdentity(withoutNamedDependency({
        package: {
