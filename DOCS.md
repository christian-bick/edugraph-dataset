...
*   **Function**: Loads targets and catalogs via `src/lib/generation.ts`, computes the complete matched `(target, generator, view)` tuple set once, groups it by generator, and reuses it for train generation, validation generation, preflight, and manifest construction. Before scoped rendering, it computes the complete dependency delta and rejects a scope that omits affected pairs. It then generates one question and one solution sample per tuple with structural seeds and renders them headlessly through the Vite server owned by the same Docker container. The final structured work-counter line exposes catalog, type-index, matching, and source-index work.
...
*   **Single-pass validation state**: After freshness succeeds, validation obtains image identities
    from immutable shard manifests, resolves checklist text once per view/path, memoizes label and
    validation contexts, and loads one `VqaCacheManager` per generator. PNG bytes are opened only
    for cache misses, forced evaluations, or the independent full-integrity audit. Pruning,
    classification, live updates, normalization, and reporting reuse the prepared state. Audit
    reads each physical cache file and each live image once. Structured counters separate VQA miss
    bytes from dataset-integrity bytes.
