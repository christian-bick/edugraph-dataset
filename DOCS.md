...
*   **Execution**: The canonical wrapper invokes `npm run generate:dataset:internal`; direct host execution is rejected. The public command is `npm run generate:dataset -- --spec=<spec_module> [--generator=<generator_name>] [--view=<view_id>] [--affected] [--training-only] [--concurrency=<positive_integer>]`.
*   **Function**: Loads targets and catalogs, builds the type-compatible module-pair index once, and resolves matched `(target, generator, view)` tuples through `src/lib/matching.ts`. With no trusted graph or after a matching-policy change it performs one complete indexed match. Otherwise it reuses unchanged successful match tuples, matches changed targets against the complete pair index, and matches changed pairs against the unchanged targets so newly enabled combinations are discovered without persisting rejected Cartesian triples. The resulting tuple set is shared by train generation, validation generation, preflight, and manifest construction. Before scoped rendering, it computes the complete dependency delta and rejects a scope that omits affected pairs. It then generates one question and one solution sample per tuple with structural seeds and renders them headlessly through the Vite server owned by the same Docker container. The final structured work-counter line exposes catalog, type-index, matching, and source-index work.
...
*   **Execution**: `npm run generate:dataset -- --spec=<spec_module> [--generator=<generator_name>] [--view=<view_id>] [--affected] [--training-only] [--concurrency=<positive_integer>]`.
...
*   **Freshness gate**: Before inspecting or spending API calls on VQA, validation rebuilds the unified dependency graph with canonical renderer identity and verifies the selected pair entries, metadata counts, content/task fingerprints, source and ontology inputs, direct edges, immutable shard image identities, and graph-owned VQA cache keys. It fails with the first causal affected path when the persisted graph is stale. A missing manifest, incomplete graph, old schema, or old planner epoch is a fail-closed miss and requires canonical regeneration. Strict audit separately reads every live image once and verifies its stored byte digest.
...
*   **Caching**: Evaluation results remain stored in `cache/vqa-validation/<dataset>/<module>.jsonl`, but cache validity is owned by the dependency graph. Every `vqa:<sample_key>` node carries the exact established key `sha256(image bytes : validation-context hash)` and depends directly on its image, applicable checklist sources, claimed ontology entities, and their definitions. Validation lookup, strict expected-set audit, pruning, and reporting all consume that graph key; the JSONL cache is output storage, not a second invalidation system. The validation context combines the applicable checklist hash with the sorted ontology labels and their definitions, so changing an image, checklist, label claim, or definition re-validates exactly the affected samples. The evaluator system instruction, response schema, and model identifier are deliberately excluded from this hash; changing any of them requires a deliberate full live validation with `--force`. Each cache entry also records the component hashes and the sample's full identity (`sample_key`, `attempt`, `seed`, …) for debugging and churn analysis. Failures in every generated timestamped report include a ready-to-run `test:sample` command.
*   **Single-pass validation state**: Graph construction memoizes checklist and label-definition contexts and derives every expected cache key from immutable image identities. After freshness succeeds, validation loads one `VqaCacheManager` per generator and resolves full checklist text, label definitions, and prompts only for live cache misses or forced evaluations; an invariant verifies that the resolved prompt context reproduces the graph key exactly. PNG bytes are opened only for those evaluations or the independent full-integrity audit. Pruning, classification, live updates, normalization, and reporting reuse the graph-owned identities. Audit reads each physical cache file and each live image once. Structured counters separate VQA miss bytes from dataset-integrity bytes.
...
*   **Function**: The unified repository check script. Orchestrates TypeScript type checking (`tsc --noEmit`), external semantic baseline validation, generator/view spec audits (`validate-generator-view-specs.ts`), label usage audits (`check-labels.ts`), documentation reference validation (`validate-docs.ts`), competency standard spec validations (`normalizeAndValidateSpec`), and dataset split integrity (`report-splits.ts`). If `--spec` is specified, restricts target spec validation to that module; otherwise validates all available specs (`test`, `ccss`). Split integrity runs only for specs whose dataset has been generated, so a fresh clone still passes every static check.
...
*   **Function**: Compares the installed pinned package with the committed ontology baseline by entity definition and individual typed relation. It also recomputes the exact CCSS label/ancestor usage closure from current targets and catalogs. The default is dry-run; `--apply` accepts the reviewed semantic delta.
