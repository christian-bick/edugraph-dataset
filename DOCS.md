...
*   `targetLabels`: Original standard target claims retained as provenance, not emitted as artifact labels and not inspected by the pure view.
...
*   `preparedView`: The admitted configuration, plan/variant identities, resolved view labels, and PRNG continuation. `withConfig` requires it and never repeats schema resolution in the browser.
...
dependencies and original-target policies share this evaluator. Matching never inspects payloads.

Plans include the complete matching input identity, including local spec imports and declared
ontology reads. Matching indexes persist plans rather than bare successful-pair references.
Manifest schema 9, dependency graph schema 5, planner epoch 6, and the
`label-variant-generation-v1` pipeline identity deliberately invalidate old artifacts. Rebuild
the full dataset before scoped reuse. Existing artifacts remain published until the replacement
transaction passes validation. Variant sampling uses a separate deterministic entropy stream;
this migration can change fallback choices and images while structural sample keys remain stable.
...
*   **Render seed**: passed to the browser as `payload.seed`. The orchestrator resolves view config once under the admitted selection and records the resulting PRNG continuation. `withConfig` restores that continuation and consumes the prepared config.
...
*   **Task fingerprint**: `computeTaskFingerprint(problem.data, resolvedViewConfig)` — an order-independent identity for what the selected view actually asks. It governs same-split reuse and target associations, so equal generator data with different view parameters remains distinct. The browser consumes exactly that prepared configuration. A retained sample can represent another target only when its actual selection is admitted by that target's plan; deduplication does not merge capabilities from different draws.
...
Operational rows retain `generation_plan_hash`, the full `generation_plan`, `generation_replay`
(actual origin sample key, attempt, seed, and selected assignment), and `prepared_view`. When a
solution reuses its question's draw, its artifact key remains the solution key and its replay
origin remains the question key. `test:sample` uses stored provenance by default; missing, corrupt,
or stale recipes fail clearly. `--attempt=<n>` explicitly requests a fresh current-plan draw.
VQA records carry the same recipe. The public merged metadata stays compact.
...
*   **Metadata**: Each standard image row records its operational identity: `sample_key`, `spec`, `target_id`, optional additional `target_associations`, `generator`, `view`, `mode`, `instance`, `attempt`, `seed`, `content_fingerprint`, `task_fingerprint`, plus `labels`. The association list preserves exact competency provenance when several targets share one physical task. The problem data supplies the content fingerprint; prepared view configuration and the complete plan/replay receipt are retained in operational metadata for exact reproduction. Generators do not author a separate descriptive id or ontology annotations; structural sample identity remains separate from content and task equivalence.
...
*   **Function**: The development delta gate. It combines committed changes from `--base`, working-tree changes, and untracked files; maps each changed root to type checking, Vitest's related-test graph, scoped implementation contracts, generator/view specs, label checks, documentation, scoped generator coverage, and affected production specs; and prints every causal file before executing. A changed `generator.ts` gets fresh related-test coverage. Its declared `ProblemGenerator<T>` output is compared with the Git baseline: a changed or unparseable contract schedules pair and active-standard checks, while a proven implementation-only edit keeps the cheap route. Implementation source checks are scoped to changed files and their module owners. Package and lock changes trigger matching/label checks and all production specs; dataset generation independently detects exact ontology provenance and reconstructs the graph. The explorer-only canonical standards tree does not trigger dataset validation. All affected production specs are validated in one process against one cached catalog. A requested full check is never an implicit fallback.
...
*   **Function**: Enforces the static generator/view contracts from [docs/spec-general.md](docs/spec-general.md), [docs/spec-generator.md](docs/spec-generator.md), and [docs/spec-view.md](docs/spec-view.md). It rejects redundant or overlapping positive declarations, incorrectly placed deduction operators, inline or prematurely executed resolvers, generator-owned Abilities, cross-pair schema parameter duplication, Abilities in `rejectTargetLabels`, unsupported pair-level `requireTargetLabels`, and contradictory required/rejected declarations. It also checks every leaf's file/export inventory and implementation source boundaries; dynamic source forms that cannot be proven receive review warnings. Label-bearing mechanisms are dimension-neutral; semantic ownership rules determine which declarations are valid. Output goes to the console; redirect it to `temp/` if you need to keep it.
*   **Shared ownership gate**: `spec-ownership.ts` supplies the same deterministic D4 diagnostics to this command and the architecture audit. It checks every module's invariant and schema declarations, including unmatched modules, and all four declaration combinations on every pair admitted by the existing compatible-pair index. Equality and specialization overlap apply across dimensions; structural relations and related schema alternatives alone are not conflicts. Each issue has a stable code and identity, rule IDs, full ontology IRIs, and exact module/declaration/parameter references. Referenced ancestry and per-module indexes are reused during the operation, with work counters covering declarations, ancestry, pair edges, and findings. The command delegates to `generator-view-spec-validation.ts`; schema export discovery shares the model catalog's naming convention, including numeric module names. Full checks, affected checks, and CI invoke this command; changes to the shared validator, discovery helper, or command implementation schedule the relevant gates.

*   **Shared applicability gate**: `spec-contracts.ts` supplies D5 diagnostics to the command and architecture audit. It groups the same compatible-pair index by view and reuses each pair's supported-label closure. Contradiction checks test whether a requirement equals or specializes a rejected label; they return both full IRIs with `SPEC-V3`/`SPEC-V7` references. Missing compatible generators, missing pair support, and Ability rejections remain failures even for views with no current matching target. Work counters record declaration, pair, rejection, and support lookups. Full, affected, and CI checks use this same gate.

### `src/scripts/validate-implementation-contracts.ts`
*   **Execution**: `npx vite-node src/scripts/validate-implementation-contracts.ts [--files=<comma-separated-paths>]`; normally run through `npm run check` or `npm run check:affected`.
*   **Function**: Checks implementation code and reachable local helpers for raw label reads, spec-driven task decisions, generator-authored annotations, and known unseeded entropy APIs. With changed files, direct entry edits scan only their modules; shared-helper edits resolve affected owners through the source graph. Nonempty generator schemas are checked for entry validation, with unproven ordering reported for review. The full spec gate also runs these checks and separately validates file/export inventory, view payload mappings, and checklist form.
...
*   **Graph reuse**: The audit loads current model catalogs because the dependency graph deliberately stores capability hashes rather than raw declaration metadata. If every current target, generator, view, matching-policy hash, target-label posting, and compatible pair agrees with the generated dataset graph, it reuses the graph's successful match tuples. Otherwise it performs one fresh indexed match. The shared model-source index discovers imported implementation closures; the same syntax-aware implementation scanner used by the gate analyzes those files. Learner-action payload-field names remain review signals, not proof of a violation. Ontology resolution in `withConfig` remains infrastructure rather than an implementation finding. Structured work counters and synthetic growth tests cover the linear tuple-index, capability-closure, provenance, overlap, import-graph, and source-read paths. No graph-schema change or dataset invalidation is required.
*   **Ownership findings**: D4 findings embed the shared `ownership` diagnostic and point to the relevant spec files. They include invariant redundancy and schema/invariant overlap within either role as well as generator Abilities and cross-role conflicts. Reusing matched tuples affects provenance reporting only; it never skips declaration checks or restricts ownership validation to currently matched targets. Existing semantic review items and heuristic signals retain their classifications.
...
The determinism rule that breaks things silently is `IMPL-V6`: every randomized visual decision must derive from `payload.seed`. Known unseeded APIs fail the source check; less obvious entropy still requires review because it can invalidate the VQA cache.
